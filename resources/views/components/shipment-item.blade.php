<section class="flex-1 grid grid-cols-8 gap-x-3 gap-y-2 items-center py-4 text-slate-700 mx-4">
    <div class="col-span-5 lg:col-span-2">
        <div class="text-center text-xs uppercase bg-slate-400 text-slate-700 font-bold rounded-t-md">
            {{ __('dhl-ui::shipment.attributes.type') }}
        </div>
        <select
    ...
    <div class="col-span-2 lg:col-span-1">
        <x-dhl-ui::diamentions label="{{__('dhl-ui::shipment.attributes.quantity')}}" model="items.{{$id}}.quantity"/>
    </div>
    ...
    <div class="col-span-2 lg:col-span-1">
        @if(isset($item['weight']))
            <x-dhl-ui::diamentions label="{{__('dhl-ui::shipment.attributes.weight')}}" model="items.{{$id}}.weight"/>
        @endif
    </div>
    <div class="col-span-2 lg:col-span-1">
        @if(isset($item['width']))
            <x-dhl-ui::diamentions label="{{__('dhl-ui::shipment.attributes.width')}}" model="items.{{$id}}.width"/>
        @endif
    </div>
    <div class="col-span-2 lg:col-span-1">
        @if(isset($item['length']))
            <x-dhl-ui::diamentions label="{{__('dhl-ui::shipment.attributes.length')}}" model="items.{{$id}}.length"/>
        @endif
    </div>
    <div class="col-span-2 lg:col-span-1">
        @if(isset($item['height']))
            <x-dhl-ui::diamentions label="{{__('dhl-ui::shipment.attributes.height')}}" model="items.{{$id}}.height"/>
        @endif
    </div>
    <div class="col-span-4 order-2">
        @if(isset($item['nonStandard']))
            <x-dhl-ui::non-standard
                label="{{__('dhl-ui::shipment.attributes.nonStandard')}}"
